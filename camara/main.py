...
def acha_mandato(ano):
    if ano in [2003,2004,2005,2006]:
        ...
        return "dilma2"
    elif ano in [1995,1996,1997,1998]:
        return "fhc1"
    elif ano in [1999,2000,2001,2002]:
        return "fhc2"
...
def conta_votacoes_mes():
    mandatos = ["fhc1","fhc2","lula1","lula2","dilma1","dilma2"]
    saida = {}
    for m in mandatos:
        props = read_csv(m+"/proposicoes.csv",sep=";")
        print(props)
        datas = list(props["DATA"])
        for d in datas:
            d = str(d)
            if d not in saida:
                saida[d] = 0
    ...
    with io.open('votacao_por_data.json', 'w', encoding="utf8") as f:
        json.dump(saida, f, ensure_ascii=False)
